...
| C2 | Reassign orphaned issues | Close #5-9 on kernel, create equivalents on tau/orchestrate. Close #10, create equivalent on tau/agent. Archive `_project/library-extraction.md`. | tau/kernel, tau/orchestrate, tau/agent |
| C3 | Phase 1 completion | Objective #2: #26 (multi-session kernel) → #27 (HTTP API with SSE streaming) → #28 (server entry point). Sequential dependency chain. | tau/kernel v0.1.0 |
| C4 | Phase 2+ | Objective #3: Skills and MCP Integration | tau/kernel |
| C5 | Phase 3+ | Objective #4: Local Development Mode | tau/kernel |
...
```
Group A (Tooling)
  A1 ──┤── All downstream work benefits from clean tooling
  A2 ──┘
         │
         v
...
Groups B and C can execute concurrently after Group A completes — they are independent until Group D, which requires both the container library (B1-B2) and the completed kernel (C3-C5).
...
| C2 | `~/tau/kernel/_project/post-extraction.md` (Project Management Updates section) |
| C3-C5 | Kernel GitHub issues + `~/tau/kernel/_project/README.md` |
